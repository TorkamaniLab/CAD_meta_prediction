...
def _make_lohrasb_obj(y_train, trainer, n_trials, measure_of_accuracy, study_name, with_stratified):
    ...
    if with_stratified == True:
        stratify = y_train[y_train.columns.to_list()[0]]
    else:
        stratify = None
...
def _make_zoish_lohrasb_pipeline(
    ...
    if trainer[0] == "cat":
        if X_train.shape[1] <= n_approx and n_features >= X_train.shape[1]: 
            ...
            shap_df = _execute_zoish_shap_selector(X_train, y_train, trainer, n_trials, n_features, measure_of_accuracy, with_stratified, 
                                                    "catboost_feature_selection", shap_n_jobs = 1) # detailed shap values for all features (saved for catboost)
    ...
    # this would keep the consistency for feature selection and avoid additivity issue
        ...
        shap_df = _execute_zoish_shap_selector(X_train, y_train, trainer, n_trials, n_features, measure_of_accuracy, with_stratified, 
                                                "approx_feature_selection", shap_n_jobs = -1, approximate = True) # approx shap values for top (approx+) N features (saved but not reuse)
        feature_selectors.append(("ColumnSelector", ColumnSelector(list(shap_df['column_name'].head(n_features)))))

    # approx first and further narrow down toward target number
    elif X_train.shape[1] > n_approx and n_features <= n_approx: 
        approx_shap_df = _execute_zoish_shap_selector(X_train, y_train, trainer, n_trials, n_approx, measure_of_accuracy, with_stratified, 
                                                        "approx_feature_selection", shap_n_jobs = -1, approximate = True) # approx shap values for all features
        shap_df = _execute_zoish_shap_selector(X_train[list(approx_shap_df['column_name'].head(n_approx))], y_train, trainer, n_trials, n_features, measure_of_accuracy, with_stratified, 
                                                "feature_selection", shap_n_jobs = 1) # detailed shap values for top (approx) N features (saved for reuse)
        feature_selectors.append(("ColumnSelector", ColumnSelector(list(shap_df['column_name'].head(n_features)))))
    ...
    # this would keep the consistency for feature selection (additivity comes)
        ...
        shap_df = _execute_zoish_shap_selector(X_train, y_train, trainer, n_trials, n_features, measure_of_accuracy, with_stratified, 
                                                "feature_selection", shap_n_jobs = 1) # detailed shap values for (approx-) all features (saved for reuse)
        feature_selectors.append(("ColumnSelector", ColumnSelector(list(shap_df['column_name'].head(n_features)))))
    ...
    # elif X_train.shape[1] <= n_approx and n_features >= X_train.shape[1]: 
        ...
        print(f"Skip feature selection - Select more features than provided. {X_train.shape[1]} {n_approx} {n_features}") 
        shap_df = pd.DataFrame({ # age+sex don't need further shap feature ranking
            "column_name": X_train.columns,
            "abs_mean_shap": [0] * len(X_train.columns) 
        })

    lohrasb_opt = _make_lohrasb_obj(y_train, trainer, n_trials, measure_of_accuracy, f"LohrasbOptimizer_{trainer[0]}_{trainer[1]}", with_stratified)
